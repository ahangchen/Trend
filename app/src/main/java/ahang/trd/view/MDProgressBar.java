...
public class MDProgressBar extends ProgressBar {
	...
	public MDProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MDProgressBar, defStyleAttr, 0);
		int backgroundColour;
		int progressColour;
		try {
			backgroundColour = ta.getColor(R.styleable.MDProgressBar_backgroundColour, 0);
			progressColour = ta.getColor(R.styleable.MDProgressBar_progressColour, 0);
			int defaultDuration = context.getResources().getInteger(android.R.integer.config_mediumAnimTime);
			duration = ta.getInteger(R.styleable.MDProgressBar_duration, defaultDuration);
		} finally {
			ta.recycle();
		}
